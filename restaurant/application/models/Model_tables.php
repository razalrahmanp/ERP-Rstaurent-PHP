...
class Model_tables extends CI_Model
{
	...
	public function getStoreActiveTable( $store_id = null )
	{
		$sql = "SELECT * FROM tables WHERE store_id = ? AND available = ? AND active = ? ORDER BY id DESC";
		$query = $this->db->query($sql, array($store_id, 1, 1));
		return $query->result_array();			
	}

	public function getStoreOrderTable( $store_id = null )
	{
		$sql = "SELECT * FROM tables WHERE store_id = ? AND active = ? ORDER BY id DESC";
