...
class Orders extends Admin_Controller 
{
	...
	*/
	public function update($id)
	{
        ...
        if ($this->form_validation->run() == TRUE) {        	
    		...
    		$this->data['stores'] = $this->model_stores->getStoresData();
        	$this->data['table_data'] = $this->model_tables->getStoreOrderTable( $orders_data[ 'store_id' ] );
	...
	*/
	...
	public function getTableByStore( $store_id = null, $type = 0 ) {
		if( $store_id ) {
			ob_start();

			if( $type == 0 ) {
				$table_data = $this->model_tables->getStoreActiveTable( $store_id );
			}
			else {
				$table_data = $this->model_tables->getStoreOrderTable( $store_id );	
			}
